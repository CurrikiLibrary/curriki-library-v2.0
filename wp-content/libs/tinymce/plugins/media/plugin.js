...
tinymce.PluginManager.add('media', function (editor, url) {
    ...
    function dataToHtml(data) {
        ...
        if (data.embed) {
            ...
            editor.settings.video_update_template_callback(data);
        } else {
    ...
    editor.on('objectResized', function (e) {
        ...
        $script_node = $(editor.selection.getNode().parentNode).find('script');
        $script_html = $script_node.html().slice(0, $script_node.html().indexOf('width')) + 'width:' + e.width + ',height:' + e.height + '});';
        $script_node.html($script_html);
    });
