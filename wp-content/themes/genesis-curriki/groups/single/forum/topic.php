...
?>
	...
	<form action="<?php bp_forum_topic_action(); ?>" method="post" id="forum-topic-form" class="standard-form">
		...
		<div class="item-list-tabs no-ajax" id="bpsubnav" role="navigation">
			<ul>
				<li><a href="#post-topic-reply" class="show-hide-new"><?php _e( 'New Reply', 'buddypress' ); ?></a></li>
				<?php if ( bp_forums_has_directory() ) : ?>
					<li><a href="<?php bp_forums_directory_permalink(); ?>"><?php _e( 'Forum Directory', 'buddypress'); ?></a></li>
				<?php endif; ?>
			</ul>
			<div class="clear"></div>
		</div>
			...
			<?php if ( bp_get_the_topic_is_last_page() ) : ?>
				<?php if ( bp_get_the_topic_is_topic_open() && !bp_group_is_user_banned() ) : ?>
					...
					<div id="post-topic-reply">
						...
						<?php do_action( 'groups_forum_new_reply_after' ); wp_nonce_field( 'bp_forums_new_reply' ); ?>
					</div>
