 ...
 */
window.community_page_go_top_active = false; 

jQuery(document).ready(function () {
    
    //{'action': 'search_analytics', 'branding':jQuery('#branding').val(), 'selectedstandardid': 'test'}
    /*
    jQuery( "#cp_search" ).autocomplete({
      source: ajaxurl+"?action=fetch_community_pages_search",
      minLength: 2,
      dataType: "jsonp",
   ...
   /*
    jQuery( "#cp_search" ).autocomplete({
      source: function( request, response ) {
        jQuery.ajax( {
          url: ajaxurl+"?action=fetch_community_pages_search",
          dataType: "json",
          data: {
            term: request.term
          },
      ...
      select: function( event, ui ) {        
        console.log( window.location.protocol+"//"+window.location.hostname+"/"+"community/"+ui.item.value );
        window.location = window.location.protocol+"//"+window.location.hostname+"/"+"community/"+ui.item.value;
      }
    } );
    */
    
    jQuery(document).on('click', 'ul#community-page-nav li a', function(event){
        event.preventDefault();
        window.community_page_go_top_active = true;
        jQuery("a#community-page-go-top").show();
        jQuery('html, body').animate({
            scrollTop: jQuery( jQuery.attr(this, 'href') ).offset().top - 110
        }, 300);
    });
    
    jQuery(document).on("click","a#community-page-go-top",function(event){        
        event.preventDefault();
        window.community_page_go_top_active = false;
        jQuery("a#community-page-go-top").hide();
        jQuery('html, body').animate({
...
jQuery(window).scroll(function() {
    ...
    if ( (jQuery(this).scrollTop() > 1 && jQuery(this).scrollTop() < 50) && window.community_page_go_top_active === true ) { //use `this`, not `document`
        jQuery('a#community-page-go-top').css({
            'display': 'none'
        });
        window.community_page_go_top_active = false; 
    }
