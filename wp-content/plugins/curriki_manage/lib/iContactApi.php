...
class iContactApi {
    ...
    public function deleteContact($iContactId) {
        // Make the call
        return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/contacts/{$iContactId}", 'delete');
    }
     ...
     * @return array
     * */
    public function getContactBy($by = 'email', $value = '') {
        /*listId
        contactId
        email
        prefix
        ...
        // Make the call and return the data
        return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/contacts?{$by}={$value}", 'GET');
    }
