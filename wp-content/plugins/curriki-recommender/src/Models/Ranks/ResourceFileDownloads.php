...
/**
 * ResourceFileDownloads model has queries that return analytics
 * These model quries AWS-Redshift/postgresql
 *
 * @author waqarmuneer
 */

class ResourceFileDownloads extends Model{
    
    use Singleton;
    
    /**
     * It return analytical stats for 'ResourceFileDownloads' based on $measures
     * 
    ...
    public function getStats(Measures $measures, $extra_params = []){
        ...
        if($measures->isAnyMeaureSet()){
            ...
            $query = "
                            ...
                            files_count,
                            download_count,
                            DENSE_RANK() over (order by download_count desc) rank 
                            from(
                                    select 
                                    resourceid,
                                    files_count,
                                    ...
                                    from (
                                            select 
                                            r.resourceid, 
                                            count(distinct rf.fileid) as files_count,
                                            count(fd.downloadid) as download_count
                                            from resources r
                                ...
                                ) tbl_resourcefiles_filedownloads
