 ...
 */
var search_mod_root = "app/modules/search/";
jQuery("document").ready(function(e){
    
    jQuery("body").on("click",".pagination-first,.pagination-previous,.pagination-num,.pagination-next,.pagination-last",function(e_pg){
        e_pg.preventDefault();        
        jQuery("#search-widget").focus();
        jQuery("#resources-wrapper").html('<img class="search-loader" src="'+search_mod_root+'assets/loader.gif?t='+t+'" alt="" />');        
        var t = new Date().valueOf();
        var request_url = encodeURIComponent(jQuery(this).attr("href"));
        jQuery.ajax({
            method: "POST",
            url: "index.php?ajax=1&action=searchpagination&search_term="+jQuery("#phrase-curriki").val()+"&t="+t+"&request_url="+request_url,
        })
        .done(function( data ) {            
            jQuery("#resources-wrapper").html(data);
        });
    });  
    
    jQuery("#site-search-form").submit(function(esf){
        esf.preventDefault();
        ...
        var t = new Date().valueOf();
        var request_url = jQuery(this).attr("action") + "&" + jQuery(this).serialize() + '&t='+t;
       jQuery.ajax({
            method: "POST",
            url: "index.php?ajax=1&action=search_form_submit&t="+ t +"&request_url=" + encodeURIComponent(request_url)
        })
        .done(function( data ) {            
            jQuery("#resources-wrapper").html(data);
        });
...
jQuery('document').ready(function () {
    ...
    var tootip_ml_obj = {"tootip_heading":"Search Tips & Advance Features"};
    
    console.log(")))))))*************))))) ", tootip_ml_obj);
...
function LtiAddResourceLink(return_url,current_url,return_type)
{    
    if( return_type === "url")
    {
