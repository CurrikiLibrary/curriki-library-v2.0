...
putenv("CLEVER_CLIENT_ID=fca743c0111b262caa25");
putenv("CLEVER_CLIENT_SECRET=1b4ab596bb6b68763e73b125c21fa37508597072");
//putenv("CLEVER_REDIRECT_BASE=53ebd470cda57e5e0a7d0c90");
putenv("CLEVER_REDIRECT_BASE=".str_replace("http://", "https://", get_bloginfo('url'))."/clever_login");
...
function process_incoming_requests($incoming_request_uri, array $options) {
  if(preg_match('/oauth/', $incoming_request_uri)) {
    try {
      ...
      echo "<h1>Curriki App Temporary Down</h1>";
      
      $firstname = $me['data']['name']['first'];
      if( $firstname === null || ( gettype($firstname)==="string" && $firstname === "" ) )
          $firstname="";      
      
      $middlename = $me['data']['name']['middle'];
      if( $middlename === null || ( gettype($middlename)==="string" && $middlename === "" ) )
          $middlename="";      
      
      $lastname = $me['data']['name']['last'];
      if( $lastname === null || ( gettype($lastname)==="string" && $lastname === "" ) )
          $lastname="";      
      
      $username = $me['data']['credentials']['district_username'];
      if( $username === null || ( gettype($username)==="string" && $username === "" ) )
          $username="";      
      
      $email = $me['data']['email'];
      if( $email === null || ( gettype($email)==="string" && $email === "" ) )
          $email="noemail".mt_rand()."@nomail.com";
      
      if($me["type"] === "school_admin" && strlen($username)===0)
      {
        $username =  "clvr-adm-".$me["data"]["id"];        
      }
      
      /*
      echo "<pre>";
      echo "<br />($firstname) ($middlename) ($lastname) ($username) ($email)<br />";      
      var_dump($me);
      die;
      */
    
      curriki_clever_authentication($email, $username, "", $firstname, $middlename, $lastname , $me);
